...
### Changed
- **`SN_TOOL_PACKAGES` default narrowed** from `all` (42 tools) to a curated subset — `core, itsm, scripts, catalog, audit` (26 tools, the "developer analytical" loop). Keeps the LLM's tool list compact out of the box; pass `SN_TOOL_PACKAGES=all` to opt back into every module, or pin a custom set. Domain-specific modules (`cmdb`, `knowledge`, `attachment`, `update_set`) are now opt-in.
