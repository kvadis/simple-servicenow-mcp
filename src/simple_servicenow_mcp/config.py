...
class Settings(BaseSettings):
    ...
    instance_url: str = Field(default="", description="ServiceNow instance URL (e.g. https://dev12345.service-now.com). May be empty when SN_INSTANCES_FILE is set.")
    auth_method: Literal["basic", "oauth"] = Field(default="basic", description="Authentication method")
    ...
    default_page_size: int = Field(default=20, ge=1, le=100, description="Default page size for list queries")
    max_page_size: int = Field(default=100, ge=1, le=1000, description="Maximum allowed page size")

    # Retry behaviour
    max_retries: int = Field(default=3, ge=0, le=10, description="Max retry attempts for retryable failures (429, 5xx, network)")
    retry_base_delay: float = Field(default=1.0, ge=0.0, le=30.0, description="Base delay in seconds for exponential backoff")
    retry_max_delay: float = Field(default=60.0, ge=1.0, le=300.0, description="Cap on any single retry wait, in seconds")
